...
namespace eosio {
ACTION
token::create(const name& issuer, const asset& maximum_supply) {
...
ACTION
token::issue(const name& to, const asset& quantity, const string& memo) {
...
ACTION
token::retire(const asset& quantity, const string& memo) {
  auto sym = quantity.symbol;
  check(sym.is_valid(), "invalid symbol name");
  check(memo.size() <= 256, "memo has more than 256 bytes");
...
ACTION
token::transfer(const name& from, const name& to, const asset& quantity, const string& memo) {
...
ACTION
token::open(const name& owner, const symbol& symbol, const name& ram_payer) {
...
ACTION
token::close(const name& owner, const symbol& symbol) {
  require_auth(owner);
  accounts acnts(get_self(), owner.value);
  auto it = acnts.find(symbol.code().raw());
  check(it != acnts.end(),
        "Balance row already deleted or never existed. "
...
ACTION
token::burn(const name& from, const asset& quantity, const string& memo) {
  require_auth(from);
